...
def pearsSim(inA, inB):
    if (len(inA) < 3):
        return 1.0
    return 0.5 + 0.5 * corrcoef(inA, inB,
                                rowvar=0)[0][1]  # 使用0.5+0.5*x 将-1，1 转为 0，1
...
def standEst(dataMat, user, simMeas, item):
    ...
    for j in range(n):
        ...
        if userRating == 0:  # 若此人未评价过该商品则不做下面处理
            continue
        overLap = nonzero(
            logical_and(dataMat[:, item].A > 0,
                        dataMat[:, j].A > 0))[0]  # 获得相比较的两列同时都不为0的数据行号
        if len(overLap) == 0:
            similarity = 0
        else:
            # 求两列的相似度
            similarity = simMeas(
                ...
                dataMat[overLap, j])  # 利用上面求得的两列同时不为0的行的列向量 计算距离
...
def recommend(dataMat, user, N=3, simMeas=cosSim, estMethod=standEst):
    unratedItems = nonzero(
        dataMat[user, :].A == 0)[1]  # 找到该行所有为0的位置（即此用户未评价的商品，才做推荐）
    ...
    return sorted(itemScores, key=lambda jj: jj[1],
                  reverse=True)[:N]  # 将推荐商品排序
